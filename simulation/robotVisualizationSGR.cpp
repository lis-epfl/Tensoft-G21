    ...
    if (argc >= 2) {
        ...
        if (std::string(argv[1]) == "file") {
            ...
            if (argc >= 15) {
                ...
                rbModel->setRotationAngle(atof(argv[8]));

                btVector3 targetPos(0.0, 0.0, 0.0);
                targetPos.setX(atof(argv[9]));
                targetPos.setZ(atof(argv[10]));
                tModel = new targetModel(targetPos);

                btVector3 rbFrontPos(0.0, 0.0, 0.0);
                rbFrontPos.setX(atof(argv[11]));
                rbFrontPos.setZ(atof(argv[12]));

                double robot_len = rbModel->getLength(atoi(argv[13]));

                double t_bearing = atof(argv[14]);

                // instantiate walls model
                walls = new obstacleModel(rbFrontPos, robot_len, targetPos, t_bearing);
            ...
            if (std::string(argv[1]) == "in") {
                ...
                rbModel = new robotModel(argc - 14, argv + 14);
                ...
                btVector3 rbFrontPos(0.0, 0.0, 0.0);
                rbFrontPos.setX(atof(argv[10]));
                rbFrontPos.setZ(atof(argv[11]));

                // robot length and bearing w.r.t. the target
                double robot_len = rbModel->getLength(atoi(argv[12]));
                double t_bearing = atof(argv[13]);
