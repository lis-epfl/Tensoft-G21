...
namespace
{
    ...
    } config =
   {
     ...
     0.31,     // radius (length)
     613.0/1.5,   //  muscle stiffness (kg / sec^2) was 1500
     200.0,    // muscle damping (kg / sec)
     16.84,     // rod_length (length)
     7.5,      // rod_space (length)
     0.99,      // friction (unitless)
     0.01,     // rollFriction (unitless)
     0.0,      // restitution (?)
     1*2452.0,        // muscle pretension -> set to 4 * 613, the previous value of the rest length controller
     0,			// History logging (boolean)
     10000000,   // muscle maxTens
     10000000,    // muscle targetVelocity
...
namespace
{
	...
	void addNodes(tgStructure& oneUnit)
    {
		...
		oneUnit.addNode(           -config.rod_space+4.4705, 0,             0,"sphere");	     // 12	sphere
		oneUnit.addNode(-config.rod_space,  0, 0);						 // 13  rigid connection to sphere

		//weightless base
		oneUnit.addNode( config.rod_space+0.2,   half_length+0.2, 0+0.2);      	   			  // 14	connected to 3 real:5
		oneUnit.addNode(0+0.2,            config.rod_space+0.2,    half_length+0.2); 		  // 15	connected to 7 real:6
		oneUnit.addNode( half_length+0.2, 0+0.2,            config.rod_space+0.2);			  // 16 connected to 9 real:10

		oneUnit.addNode( config.rod_space+0.3,   half_length+0.3, 0+0.3, "weight");      	   		  // 17	connected to 3 real:5
		oneUnit.addNode(0+0.3,            config.rod_space+0.3,    half_length+0.3, "weight"); 	  // 18	connected to 7 real:6
		oneUnit.addNode( half_length+0.3, 0+0.3,            config.rod_space+0.3,"weight");		  // 19 connected to 9 real:10
		...
		oneUnit.addNode(-config.rod_space-0.2,  -half_length-0.2, 0-0.2);            // 23 connected to 0	real:7
		oneUnit.addNode(0-0.2,           -config.rod_space-0.2,   -half_length-0.2); // 24 connected to 4	real:8
		oneUnit.addNode(-half_length-0.2, 0-0.2,           -config.rod_space-0.2);   // 25 connected to 10	real:3
	...
	void addPairs(tgStructure& oneUnit)
	{
		...
		oneUnit.addPair(14, 3, "weightless rod");
		oneUnit.addPair(15, 7, "weightless rod");
		oneUnit.addPair(16, 9, "weightless rod");

		oneUnit.addPair(23, 0, "weightless rod");
		oneUnit.addPair(4, 24, "weightless rod");
		oneUnit.addPair(25, 10, "weightless rod");
   ...
   /* void addSegments(tgStructure& finalStruct, const tgStructure& oneUnit, double edge,
    ...
    std::vector<Module> readData ()
    {
        ...
        inFile.open("./NTRTsim-master/src/dev/MovingBall_active/input_active.txt");
...
void MovingBallModel::setup(tgWorld& world)
{
	...
	//passive muscle config
	const tgBasicActuator::Config passiveMuscleConfig(config.muscle_stiffness, config.muscle_damping, config.muscle_pretension, config.hist,
						10*config.muscle_maxTens, config.muscle_targetVelocity);

	//active muscle config
	const tgBasicActuator::Config activeMuscleConfig(config.muscle_stiffness, config.muscle_damping, config.muscle_pretension, config.hist,
						10*config.muscle_maxTens, config.muscle_targetVelocity);
	...
	const tgBasicActuator::Config customMuscleConfig(config.muscle_stiffness, config.muscle_damping, force*config.muscle_pretension, config.hist,
