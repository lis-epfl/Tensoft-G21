...
MovingBallSineWaves::MovingBallSineWaves() :
    in_controller(new tgImpedanceController(1000, 613, 1)),
    //out_controller(new tgImpedanceController(100, 500, 100)),
    segments(1.0),
    //insideLength(16.5),
    //outsideLength(19.5),
    insideLength(11.27/2),
    outsideLength(19.5),
    offsetSpeed(0.0),
    cpgAmplitude(10000.0),
    cpgFrequency(5.51),
    bodyWaves(2.0),
    simTime(0.0),
    cycle(0.0),
...
void MovingBallSineWaves::applyImpedanceControlInside(const std::vector<tgBasicActuator*> stringList, double dt , std::size_t unit)
{
    for(std::size_t i = 0; i < stringList.size(); i++)
    {
    	cycle = sin(simTime * (unit+1) * cpgFrequency * 2 * M_PI / 10 );
    	        //target = 100*cycle*cpgAmplitude - 100*cpgAmplitude;
    	        target = cycle*cpgAmplitude - cpgAmplitude/2;
