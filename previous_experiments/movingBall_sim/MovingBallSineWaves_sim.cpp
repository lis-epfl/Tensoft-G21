		...
		std::size_t unit, double frequency, double amplitude, double phase)
{
    ...
    {
    	/*cycle = sin(simTime * frequency * 2 * M_PI / 10 + phase);
        ...
        double maxNoise = 0.10;
        double noise = r * ((1 + maxNoise) - (1-maxNoise)) + (1-maxNoise);


    	cycle = noise*sin(simTime * frequency * 2 * M_PI  + phase) +0.5;
