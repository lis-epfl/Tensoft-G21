...
int main(int argc, char** argv)
{
	...
	params.set(gaNnGenerations, 300);	// number of generations
	...
	for(int i=0; i<genome.length()/16;i++)
	{
		if(i%3==0)
		{
			std::cout << "f" << i/3 << "="<<genome.phenotype(i) << std::endl;
		}
		if(i%3==1)
		{
			std::cout << "amp" << i/3 << "="<<genome.phenotype(i) << std::endl;
		}
		if(i%3==2)
		{
			std::cout << "phase" << i/3 << "="<<genome.phenotype(i) << std::endl;
		}
	}
	...
	std::cout << "time: " << int(seconds/3600) <<"h" << int(double(int(seconds)%60)/60) << "min" << int(seconds)%3600 << "s" <<'\n';
...
void printInput(GAGenome & c)
{
    ...
    outFile.open("input_sim.txt");
		...
		actuatorsPerBranch[0]=int(genome.phenotype(3*13));
		actuatorsPerBranch[1]=int(genome.phenotype(3*13+1));
		actuatorsPerBranch[2]=int(genome.phenotype(3*13+2));
		actuatorsPerBranch[3]=int(genome.phenotype(3*13+3));
		...
		outFile << "order: "<<1<<" connectedModules: "<<cModules.str()<<" connectedFaces: "<<cFaces<<" freq: "<<genome.phenotype(0)<<" amplitude: "
						<<genome.phenotype(1)<<" phase: "<<genome.phenotype(2)<<std::endl;

		for(int i=0; i<actuatorsPerBranch[0] - 1 ; i++)
		{
			outFile << "order: "<<i+2<<" connectedModules: "<<i+3<<" connectedFaces: "<<1<<" freq: "<<genome.phenotype(3*i+3)<<" amplitude: "
					<<genome.phenotype(3*i+4)<<" phase: "<<genome.phenotype(3*i+5)<<std::endl;
		}

		if(actuatorsPerBranch[0])
		{
			outFile << "order: "<<actuatorsPerBranch[0]+1<<" connectedModules: "<<0<<" connectedFaces: "<<0<<" freq: "
					<<genome.phenotype(3*(actuatorsPerBranch[0]-1)+3)<<" amplitude: "<<genome.phenotype(3*(actuatorsPerBranch[0]-1)+4)<<" phase: "<<
					genome.phenotype(3*(actuatorsPerBranch[0]-1)+5)<<std::endl;
		}
		...
		for(int i=0; i<actuatorsPerBranch[1] - 1 ; i++)
		{
			outFile << "order: "<<i+actuatorsPerBranch[0]+2<<" connectedModules: "<<i+actuatorsPerBranch[0]+3<<" connectedFaces: "<<2<<" freq: "
					<<genome.phenotype(3*i+3*actuatorsPerBranch[0]+3)<<" amplitude: " <<genome.phenotype(3*i+3*actuatorsPerBranch[0]+4)<<
					" phase: "<<genome.phenotype(3*i+3*actuatorsPerBranch[0]+5)<<std::endl;
		}
		...
		if(actuatorsPerBranch[1])
		{
			outFile << "order: "<<actuatorsPerBranch[0]+actuatorsPerBranch[1]+1<<" connectedModules: "<<0<<" connectedFaces: "<<0<<" freq: "
					<<genome.phenotype(3*(actuatorsPerBranch[1]-1)+3*actuatorsPerBranch[0]+3)<<" amplitude: "
					<<genome.phenotype(3*(actuatorsPerBranch[1]-1)+3*actuatorsPerBranch[0]+4)<<" phase: "<<
					genome.phenotype(3*(actuatorsPerBranch[1]-1)+3*actuatorsPerBranch[0]+5)<<std::endl;
		}
		for(int i=0; i<actuatorsPerBranch[2] - 1 ; i++)
		{
			outFile << "order: "<<i+actuatorsPerBranch[0]+actuatorsPerBranch[1]+2<<" connectedModules: "
					...
					<<genome.phenotype(3*i+3*actuatorsPerBranch[0]+3*actuatorsPerBranch[1]+3)<<" amplitude: "
					<<genome.phenotype(3*i+3*actuatorsPerBranch[0]+3*actuatorsPerBranch[1]+4)<<" phase: "
					<<genome.phenotype(3*i+3*actuatorsPerBranch[0]+3*actuatorsPerBranch[1]+5)<<std::endl;
		}
		...
		if(actuatorsPerBranch[2])
		{
			outFile << "order: "<<actuatorsPerBranch[0]+actuatorsPerBranch[1]+actuatorsPerBranch[2]+1<<" connectedModules: "<<0
					<<" connectedFaces: "<<0
					<<" freq: "<<genome.phenotype(3*(actuatorsPerBranch[2]-1)+3*actuatorsPerBranch[0]+3*actuatorsPerBranch[1]+3)
					<<" amplitude: "<<genome.phenotype(3*(actuatorsPerBranch[2]-1)+3*actuatorsPerBranch[0]+3*actuatorsPerBranch[1]+4)
					<<" phase: "<<genome.phenotype(3*(actuatorsPerBranch[2]-1)+3*actuatorsPerBranch[0]+3*actuatorsPerBranch[1]+5)<<std::endl;
		}
		for(int i=0; i<actuatorsPerBranch[3] - 1 ; i++)
		{
			outFile << "order: "<<i+actuatorsPerBranch[0]+actuatorsPerBranch[1]+actuatorsPerBranch[2]+2<<" connectedModules: "
					...
					<<genome.phenotype(3*i+3*actuatorsPerBranch[0]+3*actuatorsPerBranch[1]+3*actuatorsPerBranch[2]+3)<<" amplitude: "
					<<genome.phenotype(3*i+3*actuatorsPerBranch[0]+3*actuatorsPerBranch[1]+3*actuatorsPerBranch[2]+4)<<" phase: "
					<<genome.phenotype(3*i+3*actuatorsPerBranch[0]+3*actuatorsPerBranch[1]+3*actuatorsPerBranch[2]+5)<<std::endl;
		}
		...
		if(actuatorsPerBranch[3])
		{
			outFile << "order: "<<actuatorsPerBranch[0]+actuatorsPerBranch[1]+actuatorsPerBranch[2]+actuatorsPerBranch[3]+1
					...
					<<" connectedFaces: "<<0
					<<" freq: "
					<<genome.phenotype(3*(actuatorsPerBranch[3]-1)+3*actuatorsPerBranch[0]+3*actuatorsPerBranch[1]+3*actuatorsPerBranch[2]+3)
					<<" amplitude: "
					<<genome.phenotype(3*(actuatorsPerBranch[3]-1)+3*actuatorsPerBranch[0]+3*actuatorsPerBranch[1]+3*actuatorsPerBranch[2]+4)
					<<" phase: "
					<<genome.phenotype(3*(actuatorsPerBranch[3]-1)+3*actuatorsPerBranch[0]+3*actuatorsPerBranch[1]+3*actuatorsPerBranch[2]+5)
					<<std::endl;
		}
		outFile.close();
}
