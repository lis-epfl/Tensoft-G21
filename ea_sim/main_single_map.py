...
from double_map.utils_double_map import evaluate_individual
from single_map import map_elites_single_map
from single_map.utils_single_map import load_checkpoint

from utils import print_header_single_map
...
def run_evo(tbox, settings, checkpoint_dir, map_evo_file, history_file, alg_data, seed, morphologies, hof,
            min_num_modules, neat_config):
    ...
    m_bd_names = ['# modules', 'Stiffness value']
    m_bd_labels = (list(range(min_num_modules, settings['robot']['max_num_modules'] + 1)),
                       settings['robot']['modules_conf']['stiff_table'])

    m_dims = tuple([len(dim_labels) for dim_labels in m_bd_labels])
    c_dims = tuple(settings['controller']['bd_shape'])

    eval_utilities = (neat_config, settings, seed, alg.get('deep_grid', False))

    pop = map_elites_single_map.run(init_sols, tbox, m_dims, m_bd_names, m_bd_labels, c_dims,
                                    map_evo_file, history_file, morphologies, hof, settings['hof_size'],
                                    settings['result_dir'], checkpoint_dir, settings['max_num_sims'],
                                    batch_size, alg.get('mutation_weights', [1, 1, 1]),
                                    maximize_fit=False, eval_utilities=eval_utilities,
...
def end_simulation(map_evo_file, history_file, results_dir, controllers_dir, hall_of_fame,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Script for simulating the evolution of morphology and controller of '
                                                 ' a modular soft robot through map elites for goal reaching '
                                                 ' or goal reaching after squeezing task')
    parser.add_argument('evo_settings', metavar='evo_settings', type=str, nargs='?',
                        default='./experiments_gr/single_map/map_elites.json', help='file name of the configuration '
                                'file containing the parameters for the simulation.' +
                                ' By default it looks for the ./experiments_gr/single_map/map_elites.json file.')
    parser.add_argument('neat_settings', metavar='neat_settings', type=str, nargs='?',
                        default='./experiments_gr/single_map//neat-settings.txt',
                        ...
                        './experiments_gr/single_map/neat-settings.txt file.')
    ...
    # print header
    print_header_single_map(evo_settings)
    ...
    for seed in seeds:
        ...
        map_evo_filename = os.path.join(evo_dir, 'map_evo_{}.json'.format(seed))
        ...
        if args.use_checkpoint and os.path.exists(checkpoint_filename):
            ...
            # restore evo files
            map_evo_file, _ = restore_evo_file(map_evo_filename, last_checkpoint, num_sims)
            ...
            # open evo files
            map_evo_file = open_evo_file(map_evo_filename)
        ...
        run_evo(toolbox, evo_settings, checkpoint_dir, map_evo_file, history_file, alg_data, seed,
                all_morphologies, hall_of_fame, robot.MIN_NUM_MODULES, c_gen_operators.neat_config)

        # perform the closing operations
        end_simulation(map_evo_file, history_file, results_dir, controllers_dir, hall_of_fame, seed,
                       all_morphologies, morphologies_dir, start_time, checkpoint_used)
